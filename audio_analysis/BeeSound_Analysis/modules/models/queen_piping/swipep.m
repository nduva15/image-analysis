...
for i = 1 : length(ws)
    ...
    L = sqrt( max( 0, interp1( f, abs(X), fERBs, 'linear', 0) ) ); %spline
    ...
    if size(Si,2) > 1
        ...
        Si = nan(length(Si), length(t)); %repmat( NaN, length(Si), length(t) );
    end
...
p = nan(size(S,2), 1); %repmat( NaN, size(S,2), 1 );
s = p;                 %repmat( NaN, size(S,2), 1 );
for j = 1 : size(S,2)
    [ s(j), i ] = max( S(:,j), [], 1 );
