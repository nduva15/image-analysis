...
function [f_LAT, f_Incr, f_Decr, f_ADSR_v] = FCalcLogAttack(f_Env_v, f_Fs, f_ThreshNoise)
my_eps = 10^(-3);  %%increase if errors occur
i_Method		= 3; % 1 = 0.8*maximum, 2 = maximum, 3 = effort
f_ThreshDecr	= 0.4;

% if ~isempty(find(isnan(f_Env_v)))
%   f_Env_v
%   pause
% end
...
for p=1:length(percent_value_v)
	pos_v				= find(f_Env_v >= percent_value_v(p)-my_eps ); %% added by DF
    ...
    if ~isempty(pos_v)  
     ...
     percent_value_v(p)
%       hold on
%       plot([1:length(f_Env_v)], percent_value_v(p), 'r-')
%      length(percent_value_v)
     [xx,I] = max(f_Env_v);
     percent_posn_v(p) = I;   
     pause
    end
...
% ==== d�tection du start (satt_posn) et du stop (eatt_posn) de l'attaque ========================
pos_v 	= find(f_Env_v > f_ThreshNoise);
switch i_Method
	case 1
		satt_posn	= pos_v(1);
		eatt_posn	= percent_posn_v(0.8/percent_step); % === �quivalent � 80%

	case 2
		satt_posn	= pos_v(1);
		eatt_posn	= percent_posn_v(1.0/percent_step); % === �quivalent � 100%
...
% === moyenne pond�r�e (gaussienne centr�e sur percent=50%) des pentes entre satt_posn et eattpos_n
...
if 0
	...
	% === moyenne pond�r�e par une gaussienne (maximum autour de 50%)
